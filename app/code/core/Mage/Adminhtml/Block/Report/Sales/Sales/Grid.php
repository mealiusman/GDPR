...
class Mage_Adminhtml_Block_Report_Sales_Sales_Grid extends Mage_Adminhtml_Block_Report_Grid_Abstract
{
    ...
    protected function _prepareColumns()
    {
        ...
        $this->addColumn('customer_group_id', array(
            ...
            //'total'         => 'sum',
            'sortable'      => false,
            //'renderer'      => 'my_reports/adminhtml_report_grid_column_renderer_customer',
            'renderer'      => 'adminhtml/report_grid_column_renderer_customergroup',
            //'rate'          => $rate,
        ));
    ...
    {
        $customerGroupIdSelectedArray = $filterData->_getData('customer_group_id');

        $customerGroupIdSelectedArray = $customerGroupIdSelectedArray[0];

        if(!empty($customerGroupIdSelectedArray)){
            //if(!in_array('1000',$customerGroupIdArray)){
            $collection->getSelect()
                //->where('customer_group_id1 IN (?)',implode("'",$customerGroupIdSelectedArray));
                ->where('customer_group_id IN ('.$customerGroupIdSelectedArray.')');
